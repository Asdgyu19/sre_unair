                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-2xl font-bold text-gray-900">{{ $eventsCount ?? 0 }}</dd>
                            </dl>
                        </div>
                    </div>
                    <div class="mt-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-2xl font-bold text-gray-900">{{ $merchandiseCount ?? 0 }}</dd>
                            </dl>
                        </div>
                    </div>
                    <div class="mt-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-2xl font-bold text-gray-900">{{ $projectsCount ?? 0 }}</dd>
                            </dl>
                        </div>
                    </div>
                    <div class="mt-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-2xl font-bold text-gray-900">{{ $postsCount ?? 0 }}</dd>
                            </dl>
                        </div>
                    </div>
                    <div class="mt-4">
            ...
            <div class="bg-white shadow-lg rounded-lg">
                ...
                <div class="p-6">
                    <div class="space-y-4 max-h-80 overflow-y-auto">
                        ...
                        {{-- Tambahkan pengecekan $event tidak null --}}
                        @if($event) 
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                <div class="flex-1">
                                    <h3 class="font-semibold text-gray-900">{{ $event->title ?? 'No Title' }}</h3>
                                    <p class="text-sm text-gray-600 mt-1">
                                        <i class="fas fa-calendar mr-1"></i>{{ $event->date ? $event->date->format('M d, Y') : 'N/A' }}
                                        <i class="fas fa-clock ml-3 mr-1"></i>{{ $event->time ?? 'N/A' }}
                                    </p>
                        ...
                        @else
                            {{-- Opsional: Pesan jika ada record event yang null --}}
                            <div class="text-center py-2 text-gray-500">Invalid event record.</div>
                        @endif
                        @empty
                    ...
                    @if(($recentEvents->count() ?? 0) > 0) {{-- Tambahkan ?? 0 --}}
            ...
            <div class="bg-white shadow-lg rounded-lg">
                ...
                <div class="p-6">
                    <div class="space-y-4 max-h-80 overflow-y-auto">
                        ...
                        {{-- Tambahkan pengecekan $item tidak null --}}
                        @if($item)
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                <div class="flex items-center flex-1">
                                    @if($item->image)
                                    <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name ?? 'Merchandise Item' }}" class="w-16 h-16 object-cover rounded-lg mr-4">
                                    @else
                                    ...
                                    @endif
                                    <div>
                                        <h3 class="font-semibold text-gray-900">{{ $item->name ?? 'No Name' }}</h3>
                                        <p class="text-sm text-gray-600">Rp{{ number_format($item->price ?? 0, 2) }}</p>
                                        <p class="text-xs text-gray-500">{{ $item->category ?? 'N/A' }} • Stock: {{ $item->stock ?? 0 }}</p>
                                    </div>
                                </div>
                                <div class="flex space-x-2 ml-4">
                        ...
                        @else
                            {{-- Opsional: Pesan jika ada record merchandise yang null --}}
                            <div class="text-center py-2 text-gray-500">Invalid merchandise record.</div>
                        @endif
                        @empty
                        <div class="text-center py-8">
                    ...
                    @if(($recentMerchandise->count() ?? 0) > 0) {{-- Tambahkan ?? 0 --}}
                    <div class="mt-4 text-center">
            ...
            <div class="bg-white shadow-lg rounded-lg">
                ...
                <div class="p-6">
                    <div class="space-y-4 max-h-80 overflow-y-auto">
                        ...
                        {{-- Tambahkan pengecekan $project tidak null --}}
                        @if($project)
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                <div class="flex-1">
                                    <h3 class="font-semibold text-gray-900">{{ $project->name ?? 'No Name' }}</h3> {{-- Menggunakan name, bukan title --}}
                                    <p class="text-sm text-gray-600 mt-1">{{ $project->category ?? 'N/A' }}</p>
                                    <div class="flex items-center mt-2">
                                        <span class="inline-block px-2 py-1 text-xs font-medium rounded-full 
                                            ...
                                            @elseif(($project->status ?? '') == 'Ditunda') bg-yellow-100 text-yellow-800
                        ...
                        @else
                            {{-- Opsional: Pesan jika ada record project yang null --}}
                            <div class="text-center py-2 text-gray-500">Invalid project record.</div>
                        @endif
                        @empty
                        <div class="text-center py-8">
                    ...
                    @if(($recentProjects->count() ?? 0) > 0) {{-- Tambahkan ?? 0 --}}
                    <div class="mt-4 text-center">
            ...
            <div class="bg-white shadow-lg rounded-lg">
                ...
                <div class="p-6">
                    <div class="space-y-4 max-h-80 overflow-y-auto">
                        ...
                        {{-- Baris yang error, tambahkan pengecekan $post->user --}}
                        @if($post)
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                <div class="flex-1">
                                    <h3 class="font-semibold text-gray-900">{{ $post->title ?? 'No Title' }}</h3>
                                    <p class="text-sm text-gray-600 mt-1">
                                        <i class="fas fa-user mr-1"></i>{{ $post->user->name ?? 'N/A' }} {{-- Perbaikan di sini --}}
                        ...
                        @else
                            {{-- Opsional: Pesan jika ada record post yang null --}}
                            <div class="text-center py-2 text-gray-500">Invalid blog post record.</div>
                        @endif
                        @empty
                        <div class="text-center py-8">
                    ...
                    @if(($recentPosts->count() ?? 0) > 0) {{-- Tambahkan ?? 0 --}}
                    <div class="mt-4 text-center">
