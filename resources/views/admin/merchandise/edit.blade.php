...
@section('content')
    <div class="mb-8">
        ...
        <p class="mt-2 text-sm text-gray-600">Update the details for "{{ $merchandise->name }}".</p>
    </div>
        ...
        <form action="{{ route('admin.merchandise.update', $merchandise->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full border-gray-300 rounded-lg shadow-sm focus:border-teal-500 focus:ring-teal-500 text-gray-900" required>{{ old('description', $merchandise->description) }}</textarea>
                @error('description') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div class="mt-6">
                <label for="status" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                <select name="status" id="status" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-teal-500 focus:ring-teal-500 text-gray-900" required>
            ...
            <div class="mt-6">
                ...
                @if($merchandise->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $merchandise->image) }}" alt="Current Image" class="w-64 rounded-lg shadow-sm border border-gray-200">
                    </div>
                @else
                ...
                <input type="file" name="image" id="image" accept="image/*"
                       class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
